...
private:
	...
	void _CodeOctTree(ostream& out, Chunk c, int rx, int ry, int rz, int a)
	{
		int u = c[rx][ry][rz];
		...
		else
		{
			...
			_CodeOctTree(out, c, rx + a / 2, ry + a / 2, rz + a / 2, a / 2);
			_CodeOctTree(out, c, rx + a / 2, ry + a / 2, rz, a / 2);
			_CodeOctTree(out, c, rx + a / 2, ry, rz + a / 2, a / 2);
			_CodeOctTree(out, c, rx + a / 2, ry, rz, a / 2);
			_CodeOctTree(out, c, rx, ry + a / 2, rz + a / 2, a / 2);
			_CodeOctTree(out, c, rx, ry + a / 2, rz, a / 2);
			_CodeOctTree(out, c, rx, ry, rz + a / 2, a / 2);
			_CodeOctTree(out, c, rx, ry, rz, a / 2);
			//递归调用
		}
		return;
	}
	...
	void _DecodeOctTree(istream& in, Chunk c, int rx, int ry, int rz, int a)
	{
		int u;
		in >> u;
		if (u == -1)//遇到非叶子节点
		{
			_DecodeOctTree(in, c, rx + a / 2, ry + a / 2, rz + a / 2, a / 2);
			_DecodeOctTree(in, c, rx + a / 2, ry + a / 2, rz, a / 2);
			_DecodeOctTree(in, c, rx + a / 2, ry, rz + a / 2, a / 2);
			_DecodeOctTree(in, c, rx + a / 2, ry, rz, a / 2);
			_DecodeOctTree(in, c, rx, ry + a / 2, rz + a / 2, a / 2);
			_DecodeOctTree(in, c, rx, ry + a / 2, rz, a / 2);
			_DecodeOctTree(in, c, rx, ry, rz + a / 2, a / 2);
			_DecodeOctTree(in, c, rx, ry, rz, a / 2);
			//递归调用
		}
		else
